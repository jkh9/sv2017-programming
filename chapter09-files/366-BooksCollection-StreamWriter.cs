...
public class BooksCollection
{
    public static void Main()
    {
        ...
        StreamWriter myFile = new StreamWriter("books.dat");
        for (int i = 0; i < books.Count; i++)
        {
            myFile.WriteLine(books[i].title);
            myFile.WriteLine(books[i].author);
            myFile.WriteLine(books[i].yearPublished.ToString());
        }
        myFile.Close();
    }
}
