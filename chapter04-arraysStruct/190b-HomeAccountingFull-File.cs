...
public class ex190b
{
    ...
    public static void Main()
    {
        ...
        {
            string[] dataFromFile = File.ReadAllLines("accounting.dat");
            for (int i = 0; i < dataFromFile.Length / 4; i++)
            {
                movements[i].date = dataFromFile[i * 4];
                movements[i].description = dataFromFile[i * 4 + 1];
                movements[i].category = dataFromFile[i * 4 + 2];
                movements[i].amount = Convert.ToDouble(
                    dataFromFile[i * 4 + 3]);
            }
            cont = dataFromFile.Length / 4;
        }
        ...
        string[] dataToFile = new string[cont * 4];
        for (int i = 0; i < cont; i++)
        {
            dataToFile[i * 4] = movements[i].date;
            dataToFile[i * 4 + 1] = movements[i].description;
            dataToFile[i * 4 + 2] = movements[i].category;
            dataToFile[i * 4 + 3] = movements[i].amount.ToString();
        }
        File.WriteAllLines("accounting.dat", dataToFile);
